...
document.addEventListener("DOMContentLoaded", function () {
  function updateDatabase(cartId, quantity) {
    $.ajax({
      ...
      error: function (xhr, status, error) {
        console.error("Error updating quantity:", xhr.responseJSON?.error || error);
      }
    });
  }
  ...
  document.querySelectorAll(".plus, .minus").forEach(button => {
    button.addEventListener("click", function () {
      const quantityInput = this.closest(".quantity-edit").querySelector(".quantity");
      ...
      // Update database and total price
      updateDatabase(cartId, quantityInput.value);
      updateTotalPrice();
    });
  });
  ...
  document.querySelectorAll(".quantity").forEach(input => {
    input.addEventListener("input", function () {
      ...
      // Update database and total price
      updateDatabase(cartId, this.value);
      updateTotalPrice();
    });
  });
